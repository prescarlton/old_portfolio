    ...
    return (
        <div className="card rounded-lg p-6 flex flex-col justify-between border-2 border-gray-200 w-full md:w-72 h-48 md:h-64 mr-4 mb-4 relative">
            ...
            <div className="links absolute right-0 top-0 p-inherit flex">
                {link && (
                    <a href={link} target="_blank" rel="noreferrer">
                        <ExternalLink className="text-gray-400 hover:text-blue-500 mb-2 md:mr-2" />
                    </a>
                )}
                {repo && (
                    <a href={repo} target="_blank" rel="noreferrer">
                        <GitHub className="text-gray-400 hover:text-blue-500" />
                    </a>
                )}
            </div>
            <ul className="flex flex-wrap">
                {tags.map((item) => {
                    return (
                        <li key={item.attributes.id} className="rounded-lg border-2 border-gray-200 mr-1 mb-1 px-1 py-1/2">
                            {item.attributes.tagText}
                        </li>
                    );
                })}
            </ul>
