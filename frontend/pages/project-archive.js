...
const repoFetcher = (...args) =>
    fetch(...args).then((res) => sortRepos(res.json()))
            ...
            <div className="flex flex-col flex-1 p-6 pb-12 md:px-36 items-stretch">
                ...
                {errorMsg && (
                    <div>
                        ...
                        <h2>
                            I'm currently using GitHub's free API so I don't
                            have to worry about authentication. The downside to
                            that is that you're limited to 60 requests / hour
                            per IP. I'm flattered that you're that interested
                            but maybe calm down just a little bit?
                        </h2>
                    </div>
                )}
            </div>
